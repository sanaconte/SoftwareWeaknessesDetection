...
def store_data(model, orig, X, just_outliers, threshold=0.5):
    outliers_file = "data/outliers.csv" #os.path.join(config.get_str('analysis', 'OutliersPath'), 'outliers.csv')
    model_type = 'CI' #config.get_str('model', 'Model')
    ...
    df = pd.DataFrame()
    #print(df)
    df[['FILE_NAME', 'LINE_NUMBER', 'NODE', 'Func', 'Var']] = orig[['FILE_NAME', 'LINE_NUMBER', 'NODE', 'Func', 'Var']]
